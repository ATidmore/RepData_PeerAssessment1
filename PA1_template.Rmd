...
```{r echo=FALSE}
#setwd("~/Training/CourseraDataScience/5ReproResearch/Week2/RepData_PeerAssessment1/activity")

library(dplyr)
library(ggplot2)
options(scipen=999) #disable scientific notation in results
```
...
```
The mean steps taken per day are `r mean_steps` and the median value is `r median_steps`.
...
```
The mean steps taken per day are `r hist_mean_steps` and the median value is `r hist_median_steps` according to the dataset with missing values extrapolated.  
